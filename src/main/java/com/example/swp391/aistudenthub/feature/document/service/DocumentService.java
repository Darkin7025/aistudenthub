 ...
 * Upload -> validate -> luu Cloudinary -> luu DB -> tra DTO.
 */
@Slf4j
@Service
...
public class DocumentService {
    ...
    public com.example.swp391.aistudenthub.feature.document.dto.response.PreviewResponse getPreview(
        ...
        // Generate signed URL if it's a Cloudinary asset to bypass PDF restrictions
        ...
        if (doc.getStoragePublicId() != null && "cloudinary".equals(doc.getStorageBucket())) {
            ...
            if (signedUrl != null) {
                // Cloudinary's generate() might not append the extension, which is needed for PDFs
                if (doc.getFileUrl().endsWith(".pdf") && !signedUrl.contains(".pdf")) {
                     signedUrl += ".pdf";
                }
        ...
        if (PreviewMode.TEXT.equals(previewMode)) {
            ...
            // PDF luôn cố gắng preview trực tiếp qua iframe từ fileUrl
            if (!StringUtils.hasText(previewUrl)) {
                // Nếu không có fileUrl, fallback sang TEXT mode nếu có extractedText
                ...
                // PDF có fileUrl -> preview trực tiếp qua iframe
                previewSupported = true;
                // Lưu extractedText để frontend có thể hiển thị tab phụ (optional)
                if (StringUtils.hasText(doc.getExtractedText())) {
                    textContent = doc.getExtractedText();
                    if (textContent.length() > TEXT_PREVIEW_LIMIT) {
                        textContent = textContent.substring(0, TEXT_PREVIEW_LIMIT);
                    ...
                    // PDF không có extracted text (có thể là scan)
                    textContent = null;
    ...
    /**
     * Stream PDF document qua backend để tránh CORS issues.
     * Backend download từ Cloudinary và stream về client.
     */
    @Transactional(readOnly = true)
    public ResponseEntity<byte[]> streamDocument(UUID documentId, com.example.swp391.aistudenthub.feature.auth.entity.User currentUser) {
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent", "Mozilla/5.0"); // Prevent Cloudinary from blocking Java User-Agent
            ...
            if (responseCode >= 400) {
                ...
                throw new AppException(ErrorCode.DOCUMENT_NOT_FOUND); // Still throw 404 so frontend handles it, but log exact cause
            }
