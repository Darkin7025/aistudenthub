...
public interface DocumentRepository extends JpaRepository<Document, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT d FROM Document d WHERE d.deletedAt IS NULL AND " +
            "d.visibility = com.example.swp391.aistudenthub.feature.document.enums.DocumentVisibility.PUBLIC AND " +
            "(:keyword IS NULL OR LOWER(d.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(d.description) LIKE LOWER(CONCAT('%', :keyword, '%'))) AND " +
            "(:subject IS NULL OR d.subject = :subject) AND " +
    ...
    @org.springframework.data.jpa.repository.Query("SELECT DISTINCT d.subject FROM Document d WHERE d.visibility = com.example.swp391.aistudenthub.feature.document.enums.DocumentVisibility.PUBLIC AND d.deletedAt IS NULL AND d.subject IS NOT NULL AND d.subject != ''")
    java.util.List<String> findDistinctPublicSubjects();

    @org.springframework.data.jpa.repository.Query("SELECT DISTINCT d.major FROM Document d WHERE d.visibility = com.example.swp391.aistudenthub.feature.document.enums.DocumentVisibility.PUBLIC AND d.deletedAt IS NULL AND d.major IS NOT NULL AND d.major != ''")
